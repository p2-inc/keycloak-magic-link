...
import static io.phasetwo.keycloak.magic.auth.util.MagicLinkConstants.AUTH_SESSION_ID;
...
public class MagicLinkContinuationAuthenticator extends UsernamePasswordForm {
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        String sessionConfirmed = singleUseStore.get(parentSessionId).get(SESSION_CONFIRMED);
        if (StringUtil.isNotBlank(sessionConfirmed)) {
            ...
            singleUseStore.put(parentSessionId,
                    validityInSecs,
                    sessionConfirmedMap
            )
            ;
            context.challenge(
                    context.form()
                            .setAttribute(AUTH_SESSION_ID, parentSessionId)
